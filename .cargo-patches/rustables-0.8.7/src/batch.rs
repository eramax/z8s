...
impl Batch {
    ...
    pub fn send(self) -> Result<(), QueryError> {
        ...
        // SAFETY: Consume the OwnedFd before passing raw fd to socket_close_wrapper
        // to prevent double-close when OwnedFd is dropped. This is a workaround for
        // rustables issue with nix 0.30+ which returns OwnedFd from socket().
        let raw_fd = sock.into_raw_fd();
        Ok(socket_close_wrapper(raw_fd, move |sock| {
