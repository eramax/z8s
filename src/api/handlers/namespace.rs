...
pub async fn list_namespaces(State(state): State<AppState>) -> Json<List<Namespace>> {
    ...
    let items: Vec<Namespace> = trackers
        ...
        .filter_map(|t| match t.resource {
            AnyResource::Namespace(ns) => Some(ns),
            _ => None,
        })
        .collect();
    Json(List {
...
pub async fn get_namespace(
    ...
    trackers
        ...
        .find_map(|t| match t.resource {
            AnyResource::Namespace(ns) if ns.metadata.name.as_deref() == Some(&name) => {
                Some(Json(ns))
            }
            _ => None,
        })
...
pub async fn create_namespace(
    ...
    state
        .apply_and_broadcast(AnyResource::Namespace(ns.clone()))
        .await
    ...
    info!(
        "Namespace verify: found={}, total_namespaces={}",
        found,
        check.len()
    );
    for t in &check {
