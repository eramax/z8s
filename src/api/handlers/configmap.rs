...
pub async fn list_configmaps_in_ns(
    state: &AppState,
    namespace: Option<String>,
) -> Json<List<ConfigMap>> {
    let items: Vec<ConfigMap> = state
        .store
...
pub async fn get_configmap(
    ...
    state
        ...
        .ok_or_else(|| {
            ApiError::not_found(format!("configmap \"{}/{}\" not found", namespace, name))
        })
}
...
pub async fn create_configmap(
    ...
    let already_exists = state.store.get_by_kind("ConfigMap").await.iter().any(|t| {
...
pub async fn update_configmap(
    ...
    state
        ...
        .map_err(|e| ApiError::bad_request(e.to_string()))?;
    match resource {
        AnyResource::ConfigMap(cm) => Ok(Json(cm)),
        _ => unreachable!(),
    }
}
