...
pub async fn list_vnets(
    ...
    let items: Vec<serde_json::Value> = state.store.get_by_kind("VNet").await
        .into_iter()
...
pub async fn create_vnet(
    ...
    let mut r: crate::netmux::crds::VNet = serde_json::from_value(body)
        .map_err(|e| ApiError::bad_request(format!("invalid VNet: {}", e)))?;
    ...
    let already_exists = state.store.get_by_kind("VNet").await.iter()
        .any(|t| t.resource.name() == r.metadata.name.as_deref().unwrap_or(""));
    let resource = AnyResource::VNet(r);
...
pub async fn get_vnet(
    ...
    let trackers = state.store.get_by_kind("VNet").await;
    for t in &trackers {
        if t.resource.name() == name {
            return Ok(Json(serde_json::to_value(&t.resource).unwrap_or_default()));
        }
    }
    Err(ApiError::not_found(format!("vnet \"{}\" not found", name)))
}
