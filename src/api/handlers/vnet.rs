...
pub async fn list_vnets(headers: axum::http::HeaderMap, State(s): State<AppState>) -> axum::response::Response {
    ...
    if let Some(items) = resp.0.get_mut("items").and_then(|v| v.as_array_mut()) {
        for item in items.iter_mut() {
            ...
            let sc = subnets.iter().filter(|t| {
                if let AnyResource::Subnet(s) = &t.resource { s.spec.vnet == name } else { false }
            }).count();
    ...
    if crate::api::handlers::crd::wants_table(&headers) {
        let cols = &[("CIDR", ".spec.cidr", "string"), ("Role", ".spec.role", "string"),
        ...
        return (axum::http::StatusCode::OK, Json(build_table(resp.0["items"].as_array().unwrap_or(&vec![]), cols))).into_response();
    }
