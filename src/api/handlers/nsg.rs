...
pub async fn list_nsgs(
    ...
    let items: Vec<serde_json::Value> = state.store.get_by_kind("NSG").await
        .into_iter()
...
pub async fn create_nsg(
    ...
    let mut r: crate::netmux::crds::Nsg = serde_json::from_value(body)
        .map_err(|e| ApiError::bad_request(format!("invalid NSG: {}", e)))?;
    ...
    let already_exists = state.store.get_by_kind("NSG").await.iter()
        .any(|t| t.resource.name() == r.metadata.name.as_deref().unwrap_or(""));
    let resource = AnyResource::Nsg(r);
...
pub async fn get_nsg(
    ...
    let trackers = state.store.get_by_kind("NSG").await;
    for t in &trackers {
        if t.resource.name() == name {
            return Ok(Json(serde_json::to_value(&t.resource).unwrap_or_default()));
        }
    }
    Err(ApiError::not_found(format!("nsg \"{}\" not found", name)))
}
