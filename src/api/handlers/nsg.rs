...
pub async fn list_nsgs(headers: axum::http::HeaderMap, State(s): State<AppState>) -> axum::response::Response {
    ...
    if let Some(items) = resp.0.get_mut("items").and_then(|v| v.as_array_mut()) {
        for item in items.iter_mut() {
            ...
            let targets = item["spec"]["target_vnets"].as_array()
    ...
    if crate::api::handlers::crd::wants_table(&headers) {
        let empty = vec![];
        let cols = &[("Targets", ".spec._targets", "string"), ("Rules", ".spec._ruleCount", "integer"),
                     ("Allows", ".spec._allowCount", "integer"), ("Denies", ".spec._denyCount", "integer")];
