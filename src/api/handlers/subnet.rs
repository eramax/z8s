...
fn ip_in_cidr(ip: &str, cidr: &str) -> bool {
    ...
    let prefix_len: u32 = match parts[1].parse() { Ok(n) => n, Err(_) => return false };
    ...
    if ip_parts.len() != 4 || cidr_parts.len() != 4 { return false; }
...
pub async fn list_subnets(headers: axum::http::HeaderMap, State(s): State<AppState>) -> axum::response::Response {
    ...
    if let Some(items) = resp.0.get_mut("items").and_then(|v| v.as_array_mut()) {
        for item in items.iter_mut() {
            ...
            let pc = s.process_tracker.pod_ips().await.iter()
                .filter(|(_, ip)| ip_in_cidr(&ip.to_string(), cidr))
                .count();
            let sc = svcs.iter().filter(|t| {
                if let AnyResource::Service(svc) = &t.resource {
                    let subnet_ann = svc.metadata.annotations.as_ref()
    ...
    if crate::api::handlers::crd::wants_table(&headers) {
        let empty = vec![];
        let cols = &[("CIDR", ".spec.cidr", "string"), ("VNet", ".spec.vnet", "string"),
                     ("Pods", ".spec._podCount", "integer"), ("Services", ".spec._serviceCount", "integer")];
