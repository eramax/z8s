...
pub async fn list_routetables(headers: axum::http::HeaderMap, State(s): State<AppState>) -> axum::response::Response {
    let mut resp = crate::api::handlers::crd::generic_list(&s, "RouteTable", "RouteTableList").await;
    if let Some(items) = resp.0.get_mut("items").and_then(|v| v.as_array_mut()) {
        for item in items.iter_mut() {
            ...
            let allows = item["spec"]["rules"].as_array().map_or(0, |a|
                a.iter().filter(|r| r["action"].as_str() == Some("allow")).count());
            let denies = item["spec"]["rules"].as_array().map_or(0, |a|
                a.iter().filter(|r| r["action"].as_str() == Some("deny")).count());
            let methods: Vec<String> = item["spec"]["rules"].as_array().map_or(vec![], |a| {
                a.iter().filter_map(|r| {
                    let action = r["action"].as_str().unwrap_or("");
                    let methods = r["methods"].as_array()
                        .map(|m| m.iter().filter_map(|m| m.as_str()).collect::<Vec<_>>().join(","))
                        .unwrap_or_default();
                    if methods.is_empty() { None } else { Some(format!("{action}: {methods}")) }
                }).collect()
            });
    ...
    if crate::api::handlers::crd::wants_table(&headers) {
        let empty = vec![];
        let cols = &[("Rules", ".spec._ruleCount", "integer"), ("Allows", ".spec._allowCount", "integer"),
                     ("Denies", ".spec._denyCount", "integer")];
        return (StatusCode::OK, Json(build_table(resp.0["items"].as_array().unwrap_or(&empty), cols))).into_response();
    }
