            ...
            if let Some(http) = &rule.http {
                for path in &http.paths {
                    let port = path.backend.service.as_ref()
                        .and_then(|svc| svc.port.as_ref()).and_then(|p| p.number).unwrap_or(80) as u16;
                    let service_name = path.backend.service.as_ref().map(|s| s.name.clone()).unwrap_or_default();
                    routes_by_host.insert(host.to_string(), (service_name, port));
                }
            }
        }
    }
    state.routes.write().unwrap_or_else(|e| { tracing::warn!("rwlock poisoned"); e.into_inner() }).insert(uid.to_string(), routes_by_host);
    Ok(())
}

pub fn remove_ingress(state: &IngressState, ingress: &Ingress) -> Result<()> {
    let uid = ingress.metadata.uid.as_deref().unwrap_or("");
    state.routes.write().unwrap_or_else(|e| { tracing::warn!("rwlock poisoned"); e.into_inner() }).remove(uid);
    Ok(())
}
...
async fn handle_connection(mut client: TcpStream, state: &IngressState, store: &dyn StoreBackend) -> Result<()> {
    ...
    let backend = {
        ...
        routes.values().find_map(|m| m.get(host).or_else(|| m.get("*")).cloned())
    };
    let (backend_host, backend_port) = match backend {
        Some((ref svc, p)) => resolve_endpoint(store, svc, p).await,
        None => return Ok(()),
    };
    let mut backend = TcpStream::connect(format!("{}:{}", backend_host, backend_port)).await.context("connect backend")?;
        ...
        if let AnyResource::Service(svc) = &t.resource {
            if svc.metadata.name.as_deref() == Some(service_name) {
                if let Some(ip) = svc.spec.as_ref().and_then(|s| s.cluster_ip.as_deref()) {
                    if !ip.is_empty() && ip != "None" { return (ip.to_string(), port); }
                }
            }
        }
    }
