            ...
            if !cluster_ip.is_empty() && cluster_ip != "None" {
                ...
                let container_port = match &target_port {
                    ...
                    IntOrString::String(name) => {
                        ...
                        Self::resolve_named_port(self.store.as_ref(), &svc_ns, name)
                            .await
                            .unwrap_or(svc_port_num)
                    }
                };

                // Resolve backend pods matching the selector
                let backends = Self::resolve_backend_pods(
                    self.store.as_ref(),
                ...
                .await;
                debug!(
                    "resolve_backend_pods for {}: found {} backends",
                    key,
                    backends.len()
                );
                ...
                debug!(
                    "Service {} → ClusterIP {} — adding DNAT with {} backends",
                    key,
                    listen_addr,
                    backends.len()
                );
                if let Err(e) = self
                    .netmux
                    .nft
                    .add_dnat(cluster_ip_addr, svc_port_num, &backends)
                    .await
                {
                ...
                if let Some(node_port) = svc_port.node_port.map(|p| p as u16) {
                    ...
                    let container_port = match &target_port {
                        ...
                        IntOrString::String(name) => {
                            Self::resolve_named_port(self.store.as_ref(), &svc_ns, name)
                                .await
                                .unwrap_or(svc_port.port as u16)
                        }
                    };
                    let backends = Self::resolve_backend_pods(
                        self.store.as_ref(),
                    ...
                    .await;
                    let cluster_ip_addr: std::net::Ipv4Addr =
                        cluster_ip.parse().unwrap_or_else(|_| {
                            debug!("ClusterIP for {} is empty, using default", key);
                            std::net::Ipv4Addr::new(10, 96, 0, 1)
                        });
                    debug!(
                        "Service {} → NodePort {} — adding DNAT with {} backends",
                        key,
                        listen_addr,
                        backends.len()
                    );
                    if let Err(e) = self
                        .netmux
                        .nft
                        .add_dnat(cluster_ip_addr, svc_port.port as u16, &backends)
                        .await
                    {
                        tracing::error!("add_dnat failed for {}: {:?}", key, e);
                    }
                    tracing::info!(
                        "calling add_nodeport_dnat for {}: node_port={}, backends={}",
                        key,
                        node_port,
                        backends.len()
                    );
                    if let Err(e) = self
                        .netmux
                        .nft
                        .add_nodeport_dnat(node_port, &backends)
                        .await
                    {
